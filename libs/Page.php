	...
	class Page{
		...
		public static function makePage($rowCount,$pageSize){
			...
			if($pages>0){
				...
				if($p!=1){
					//                a=aaa&b=bbb&flag=1&
					$pageStr .="<a href='?{$old}page=1'>首页</a>";
					$pageStr .="<a href='?{$old}page=".($p-1)."'>上一页</a>";
				...
				if($p<=5){
					...
					$end = $p+2;
					//拼接页面1、2
					$pageStr .="<a href='?{$old}page=1'>1</a>";
					$pageStr .="<a href='?{$old}page=1'>2</a>";
					$pageStr .="<a >...</a>";
				...
				for($i=$start;$i<=$end;$i++){
					...
					$pageStr .="<a href='?{$old}page={$i}' {$str} >{$i}</a>";
				}
				//判断如果 $end 小于 总页数，添加...
				if($end < $pages){
				...
				//如果当前页小于总页数，显示下一页和末页
				if($p<$pages){
					$pageStr .="<a href='?{$old}page=".($p+1)."'>下一页</a>";
					$pageStr .="<a href='?{$old}page=".$pages."'>末页</a>";
				}

				$pageStr .= "<span>共<b>{$pages}</b>页到第</span>";
				$pageStr .= "<form action='' style='display:inline-block;'><input type='text' class='txt' name='page' value='{$p}'>";
				$pageStr .= "<span>页</span>";
				$pageStr .= $inputStr;
